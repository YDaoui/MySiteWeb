document.addEventListener('DOMContentLoaded', function () {
    ...
    document.querySelectorAll('a[href^="#"]').forEach(anchor => {
        anchor.addEventListener('click', function (e) {
            ...
            if (navbar && navbar.classList.contains('active')) {
                ...
                // IMPORTANT: Supprimez cette section. Laissez le CSS gérer la transition.
    ...
    // --- Mobile menu toggle (Déplacé ici et non dupliqué) ---
    const mobileToggle = document.querySelector('.mobile-menu-toggle');
    const navbar = document.querySelector('.navbar');
    if (mobileToggle && navbar) {
        mobileToggle.addEventListener('click', () => {
            ...
            // Laisser le CSS gérer cela via transition-delay sur .navbar.active ul li
            // Mais si vous voulez une réinitialisation explicite quand ça ferme:
            if (!navbar.classList.contains('active')) { // Si on ferme le menu
                document.querySelectorAll('.navbar ul li').forEach((item) => {
                    item.style.transition = 'none'; // Désactiver les transitions pour la réinitialisation rapide
                    item.style.opacity = '0';
                    item.style.transform = 'translateY(-20px)';
                    // Re-activer les transitions après un court délai pour la prochaine ouverture
                    setTimeout(() => {
                        item.style.transition = ''; // Remettre la transition par défaut définie en CSS
                    }, 10);
                });
            }
        });
    ...
    function animateOnScroll() {
        const elements = document.querySelectorAll('.service-card, .project-card, .tech-item, [data-animate]'); // Inclure data-animate
        ...
        elements.forEach(element => {
            ...
            if (elementTop < windowHeight - elementVisible) {
                element.classList.add('animate'); // Utilisez 'animate' pour data-animate
                // Si vous avez des styles inline pour d'autres éléments, ajustez-les ou supprimez-les ici
